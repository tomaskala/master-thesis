...
\paragraph{Acceptance ration computation}
...
& = \min \left\{1, \frac{\aux^\prime p(\btheta^\prime) \psi(\aux^\prime \mid \btheta^\prime, \by_{1:T})}{\aux^{(m-1)} p(\btheta^{(m-1)}) \psi(\aux^{(m-1)} \mid \btheta^{(m-1)}, \by_{1:T})} \frac{\prop(\btheta^{(m-1)} \mid \btheta^\prime) \psi(\aux^{(m-1)} \mid \btheta^{(m-1)}, \by_{1:T})}{\prop(\btheta^\prime \mid \btheta^{(m-1)}) \psi(\aux^\prime \mid \btheta^\prime, \by_{1:T})} \right\} \\
& = \min \left\{1, \frac{\aux^\prime p(\btheta^\prime)}{\aux^{(m-1)} p(\btheta^{(m-1)})} \frac{q(\btheta^{(m-1)} \mid \btheta^\prime)}{q(\btheta^\prime \mid \btheta^{(m-1)})} \right\}.
\end{split}
\end{equation*}
